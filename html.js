const parsers = {
  INT, STR, DOT, VAL, GROUP, NAME, OP, CALL, SET, FUNCTION, ARGUMENT, ARGUMENTS, IF, ELIF, ELSE, TRUE, FALSE, WHILE, GET
};
...
function OP(elem) {
  return parseChildren(elem).join(
    elem.getAttribute("_") ?? elem.attributes[0].name
  );
}

function CALL(elem) {
  return (
    parseElem(elem.children[0]) +
    "(" +
    parseChildren(elem).slice(1).join() +
    ")"
  );
}
...
function SET(elem) {
  if (elem.children[0].tagName === "NAME") {
    return (
      parseElem(elem.children[0]) + "=" + parseElem(elem.children[1]) + ";"
    );
  } else {
    return (
      (elem.getAttribute("_") ?? elem.attributes[0].name) +
...
function getData(e) {
  ...
  switch (e.tagName) {
    case "SET":
      console.log(e, e.children[0].tagName);
      if (e.children[0].tagName === "NAME") {
        console.warn(
          getData(e.children[0]) + "=" + getData(e.children[1]) + ";"
        );
        content += getData(e.children[0]) + "=" + getData(e.children[1]) + ";";
      } else {
        console.info("err");
        content += e.attributes[0].name + "=" + read(e) + ";";
        break;
      }
    default:
      content += e.childNodes.length > 0 ? read(e) : "|";
      console.log(
        ">>>",
        e.tagName || e.textContent,
        e.childNodes.length,
        [...e.childNodes].map((e) => e.tagName)
      );
  }
...
document.querySelectorAll("do").forEach((e) => eval(parseChildren(e).join("")));
